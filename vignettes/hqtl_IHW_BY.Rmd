...
```
```{r eval=FALSE}
ggsave(cov_scatter_gg, filename="cov_scatter_gg.pdf", width=6,height=4)
```
...
gg_marginal_hist <- ggplot(chr1_chr2_df, aes(x=pvalue*10^4)) + 
         geom_histogram(aes(y=..density..), alpha=0.5, binwidth=0.05, boundary = 0, colour="black",fill=pretty_colors[1]) +
          ...
          xlab(expression(paste("p-value (", 10^-4, ")"))) 
gg_marginal_hist
```
```{r eval=FALSE}
ggsave(gg_marginal_hist, filename="gg_marginal_hist.pdf", width=6,height=4)
```
...
```{r eval=FALSE}
ggsave(gg_stratified_hist, filename="gg_stratified_hist.pdf", width=12,height=4)
```


# Apply IHW-BY and BY

We want to apply the Benjamini-Yekutieli at α=0.1, thus we will apply Benjamini-Hochberg at the corrected level:
...
# Pick out some of the hypotheses

For our table we need we one hypothesis in Chr1 that gets rejected both times (by BH and IHW):
```{r}
