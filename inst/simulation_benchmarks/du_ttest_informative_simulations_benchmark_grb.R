...
#-----------------------------------------------------------------------------
dir.create("result_files/du_ttest_informative_simulations_benchmark_grb/")
eval_table <- run_evals(sim_funs, fdr_methods, nreps, alphas, BiocParallel=T, print_dir = "result_files/du_ttest_informative_simulations_benchmark_grb/")
eval_table <- mutate(eval_table,
		eff_size = sapply(strsplit(eval_table$sim_pars,"effect size:"),
				function(x) as.numeric(x[2])))

saveRDS(eval_table, file="result_files/du_ttest_informative_simulation_benchmark_grb.Rds")
